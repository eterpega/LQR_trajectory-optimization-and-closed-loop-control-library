function [lqr, u_cl_fun, x0_p, u0_p, tIdxFun] = tvLqr(sys, lqr, tspan, x0, u0)
...
%     Alin_t = @(t) eval(subs(A_lin_sym, [num2cell(sys.stateVars)', paramNames'], ...
%         [x0_p(tIdxFun(t)) paramVals]));
%     Blin_t = @(t) eval(subs(B_lin_sym, paramNames', paramVals));
%     Alin_t = @(t) eval(subs(A_lin_sym, [sys.stateVars; sys.inputVars], [x0_p(tIdxFun(t))'; u0_p(tIdxFun(t))]));
%     Blin_t = @(t) eval(subs(B_lin_sym, sys.stateVars, x0_p(tIdxFun(t))'));
